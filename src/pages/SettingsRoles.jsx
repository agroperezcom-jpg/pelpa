...
import { Shield, Plus, Edit, Trash2, Lock, CheckCircle2, Users } from "lucide-react";
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">
          Gestión centralizada de roles y control de acceso
        </p>
      </div>
      ...
      <div className="flex items-center justify-between">
        <div className="text-sm">
          <p className="font-medium text-foreground">Total roles</p>
          <p className="text-muted-foreground text-xs mt-0.5">{roles.length}</p>
        </div>
        <Button onClick={() => handleOpenRolDialog()} className="bg-primary hover:bg-primary/90">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="p-3 bg-secondary/50 rounded-lg">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-xs text-muted-foreground font-medium">Permisos</span>
                  </div>
      ...
      <Dialog open={permisosDialogOpen} onOpenChange={setPermisosDialogOpen}>
        <DialogContent className="max-w-5xl max-h-[85vh] overflow-y-auto p-0">
            ...
            {['Operativa', 'Finanzas', 'Gestión', 'Contactos', 'Análisis', 'Sistema'].map(categoria => {
              ...
              return (
                <div key={categoria} className="space-y-4">
                  <h3 className="text-sm font-semibold text-foreground uppercase">{categoria}</h3>
          ...
          <div className="sticky bottom-0 bg-card border-t px-6 py-4 space-y-3">
            ...
            <div className="flex justify-end gap-3">
              <Button 
                variant="outline" 
                onClick={() => {
                  if (hayChangesPendientes && !window.confirm('¿Descartar cambios?')) return;
                  setPermisosDialogOpen(false);
                }}
              >
                Cancelar
